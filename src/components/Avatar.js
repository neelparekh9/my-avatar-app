...
const AvatarModel = ({ avatarUrl, currentAnimation, setCurrentAnimation }) => {
  ...
  // Use animations
  const { actions, mixer } = useAnimations([waveAnimation[0], nodYesAnimation[0], shakeNoAnimation[0], idleAnimation[0]], group);
  ...
  useEffect(() => {
    ...
    return () => {
      mixer.removeEventListener('finished', () => setCurrentAnimation("Idle"));
    };
